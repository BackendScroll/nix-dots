...
# kde-material-you-colors is not packaged in nixpkgs and is not installed
# declaratively. Theming Qt/KDE applications is optional, so skip it rather
# than failing the whole wallpaper switch.
if ! command -v kde-material-you-colors >/dev/null 2>&1; then
    echo "kde-material-you-colors is not installed; skipping KDE theming." >&2
    exit 0
fi
