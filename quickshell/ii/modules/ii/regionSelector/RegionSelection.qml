...
PanelWindow {
    ...
    readonly property real monitorScale: hyprlandMonitor?.scale ?? screen?.devicePixelRatio ?? 1
    ...
    readonly property list<var> layerRegions: {
        const layersOfThisMonitor = root.hyprlandMonitor
            ? root.layers[root.hyprlandMonitor.name]
            : null
    ...
    TempScreenshotProcess {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0) {
                const details = screenshotErrorCollector.text.trim();
                console.warn(
                    "[Region Selector] Temporary screenshot failed:",
                    exitCode,
                    exitStatus,
                    details
                );
                Quickshell.execDetached([
                    "notify-send",
                    "QuickShell screenshot failed",
                    details.length > 0 ? details : "grim could not capture the selected output",
                    "-u", "critical",
                    "-a", "QuickShell"
                ]);
                root.dismiss();
    ...
    Process {
        ...
        stdout: StdioCollector {
            ...
            onStreamFinished: {
                ...
                try {
                    ...
                    console.warn(
                        "[Region Selector] Ignoring invalid image-region output:",
                        error,
                        output
                    );
                    root.imageRegions = [];
        ...
        stderr: StdioCollector {
            id: imageDetectionErrorCollector
        }
        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0) {
                console.warn(
                    "[Region Selector] Content-region detection unavailable:",
                    exitCode,
                    exitStatus,
                    imageDetectionErrorCollector.text.trim()
                );
                root.imageRegions = [];
            }
        }
    ...
    function snip() {
        ...
        // Ignore stale persisted Desktop paths.
        ...
        if (!command || command.length === 0) {
            console.warn("[Region Selector] No screenshot command was generated.");
            root.dismiss();
            return;
        }
