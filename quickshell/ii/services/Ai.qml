...
Singleton {
    ...
    property list<var> availableTools: {
        const apiFormat = models[currentModelId]?.api_format;
        const toolsForFormat = apiFormat ? root.tools[apiFormat] : null;
        return toolsForFormat ? Object.keys(toolsForFormat) : [];
    }
