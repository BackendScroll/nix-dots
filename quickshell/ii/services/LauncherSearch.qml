    ...
    property list<var> results: {
        ...
        const commandResultObject = resultComp.createObject(null, {
            ...
            execute: () => {
                ...
                const needsTerminal =
                    cleanedCommand === "sudo"
                    || cleanedCommand.startsWith("sudo ");
                ...
                Quickshell.execDetached([
                    ...
                    needsTerminal
                        ? (
                            `${Config.options.apps.terminal} fish -C '`
                            + StringUtils.shellSingleQuoteEscape(cleanedCommand)
                            + "'"
                        )
                        : cleanedCommand
                ]);
        ...
        const webSearchResultObject = resultComp.createObject(null, {
            ...
            iconType: LauncherSearchResult.IconType.Material,
            execute: () => {
                const query = StringUtils.cleanPrefix(
                    root.query,
                    Config.options.search.prefix.webSearch
                );
                const excludedSites =
                    Config.options.search.excludedSites.map(
                        site => `-site:${site}`
                    );
                const terms = [query, ...excludedSites]
                    .filter(term => term.length > 0)
                    .join(" ");
                const url =
                    Config.options.search.engineBaseUrl
